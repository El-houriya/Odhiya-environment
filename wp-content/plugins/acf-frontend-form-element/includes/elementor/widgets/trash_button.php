...
class Delete_Post_Widget extends Widget_Base
{
    ...
    protected function render()
    {
        ...
        $redirect_url = home_url( add_query_arg( $redirect_query, $wp->request ) );
        $product = ( isset( $settings['main_action'] ) && $settings['main_action'] == 'edit_product' ? '_product' : '' );
        switch ( $settings['delete_redirect' . $product] ) {
            case 'custom_url':
                if ( $settings['redirect_after_delete' . $product]['url'] ) {
                    $redirect_url = add_query_arg( $redirect_query, $settings['redirect_after_delete' . $product]['url'] );
                }
                break;
            case 'referer_url':
                $redirect_url = add_query_arg( $redirect_query, wp_get_referer() );
                break;
        }
