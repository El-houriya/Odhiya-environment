...
class ACFEF_Payments{
	...
	public function credit_card_scripts( $processor ){
		wp_enqueue_style( 'acfef-card', ACFEF_URL . 'includes/assets/css/card.css', array(), ACFEF_ASSETS_VERSION );
		wp_enqueue_script( 'acfef-card', ACFEF_URL . 'includes/assets/js/card.js', array(), ACFEF_ASSETS_VERSION );	
		wp_enqueue_script( 'acfef-credit-card', ACFEF_URL . 'includes/assets/js/credit-card.js', array( 'acfef-card' ), ACFEF_ASSETS_VERSION );	
		$payments_depends = [];
		if( $processor == 'stripe' ){
			wp_enqueue_script( 'stripe-js', 'https://js.stripe.com/v2/', [], ACFEF_ASSETS_VERSION, true );
			$payments_depends[] = 'stripe-js';
		}
		wp_enqueue_script( 'acfef-payments', ACFEF_URL . 'includes/assets/js/payments.js', $payments_depends, ACFEF_ASSETS_VERSION, true );	
